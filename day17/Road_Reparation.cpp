...
typedef vector<vector<pll>> vec2p;
vec presum(vec &nums)
{
    vec prefix(nums);
...
void bfs(vec &dist, vec2 &adj, ll node)
{
    queue<pll> q;
    q.push({node, 0});
    vec vis(adj.size(), 0);
    ...
    while (!q.empty())
    {
        ...
        for (auto val : adj[now.first])
        {
            if (!vis[val])
            {
                q.push({val, now.second + 1});
                vis[val] = 1;
            }
        }
    }
...
int main()
{
    ...
    ll sum = 0;
    priority_queue<pll,vecp,greater<pll>> pq;
    vec vis(n+1,0);
    pq.push({0,1});
    ll cnt = 0;
    while(!pq.empty()){
