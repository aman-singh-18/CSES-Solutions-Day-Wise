...
vec ans(vector<vecp> &adj, ll node, ll k)
{
    ll n = adj.size() - 1;
    priority_queue<pll, vector<pll>, greater<pll>> q;
    q.push({0, 1});
    vector<multiset<ll>> v(n + 1);
    v[1].insert(0);
    while (!q.empty())
    {
        pll now = q.top();
        ...
        ll node = now.second;
        ll di = now.first;
        if (v[node].size() == k && di > *v[node].rbegin())
            continue;
        for (auto val : adj[now.second])
        {
            if (v[val.first].size() < k)
            {
                v[val.first].insert(di + val.second);
                q.push({di + val.second, val.first});
            }
            else
            {
                auto it = v[val.first].end();
                it--;
                if (*it > (di + val.second))
                {
                    v[val.first].erase(it);
                    v[val.first].insert(di + val.second);
                    q.push({di + val.second, val.first});
                }
            }
