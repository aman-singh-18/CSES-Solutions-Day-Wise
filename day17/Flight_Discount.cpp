...
void ans(vector<vecp> &adj, vec2 &dist)
{
    priority_queue<pro, vector<pro>, greater<pro>> q;
    q.push({0, 1, 0});
    while (!q.empty())
    {
        pro now = q.top();
        q.pop();
        if(dist[now.s][now.t]!=now.f) continue;
        if (now.t == 1)
        {
            for (auto val : adj[now.s])
            {
                if (now.f + val.second < dist[val.first][1])
                {
                    dist[val.first][1] = now.f + val.second;
                    q.push({dist[val.first][1], val.first, 1});
                }
            }
        }
        else
        {
            for (auto val : adj[now.s])
            {
                if (now.f + val.second < dist[val.first][0])
                {
                    dist[val.first][0] = now.f + val.second;
                    q.push({dist[val.first][0], val.first, 0});
                }
                if ((now.f + val.second/2) < dist[val.first][1])
                {
                    dist[val.first][1] = now.f + val.second/2;
                    q.push({dist[val.first][1], val.first, 1});
                }
            }
        }
...
int main()
{
    ...
    vec2 dist(n + 1, vec(2, 100000000000000));
    dist[1][0] = 0;
    ans(adj, dist);
