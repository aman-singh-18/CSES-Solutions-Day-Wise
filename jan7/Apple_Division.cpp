...
public:
    ...
    void update(ll idx, ll low, ll high, ll pos, long long val)
    {
        if (low == high)
        {
            seg[idx] = val;
...
bool customCompare(const pll &a, const pll &b)
{
    if (a.first != b.first)
    {
        return a.first < b.first; // Increasing first element
    }
    return a.second > b.second; // Decreasing second element
}

bool suitable(vec &v, ll maxi, ll k)
{
    ll sum = 0;
    ll cnt = 1;
    bool ya = true;
...
ll ans(ll idx,vec &v,ll sum){
    if(idx==(v.size())){
        return abs(totsum-2*sum);
    }
    return min(ans(idx+1,v,sum+v[idx]),ans(idx+1,v,sum));
}
...
{   
   ...
   totsum = sum_total(v);
   cout<<min(ans(1,v,0),ans(1,v,v[0]))<<endl;
}