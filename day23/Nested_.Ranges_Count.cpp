...
bool cmpnod(const nod &a, const nod &b)
{
    if (a.fi != b.fi)
        return a.fi < b.fi;
    return a.se > b.se;
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
       some[v[i].th] = pbs1.size()-pbs1.order_of_key({v[i].se,-1});
       pbs1.insert({v[i].se,i});
    }
    for(int i=n-1;i>=0;i--){
        its[v[i].th] = pbs2.order_of_key({v[i].se,LLONG_MAX});
        pbs2.insert({v[i].se,i});
    }
    print(its);
    print(some);
    return 0;
