...
ll ans(vec2 &adj)
{
    ...
    for (int i = st; i < topo.size(); i++)
    {
        for (auto val : adj[topo[i]])
        {
            dp[val] = (dp[val] + dp[topo[i]]) % mod;
        }
    }
    return dp[n];
}
...
int main()
{
  ...
  ll od=0;
  for(auto val:m) od+=((val.second%2)!=0);
  if(od>1) cout<<"NO SOLUTION"<<endl;
  else if(od==1 && s.size()%2==0 ){
     ...
     for(auto val:m){
        for(int i=0;i<val.second;i++){
            if(i%2==0) dq.push_back(val.first);
            else dq.push_front(val.first);
        }
     }
