...
void bfs(vector<string> &v, pair<ll, ll> a, pair<ll, ll> b, vec2 &vis, vector<vector<pair<ll, ll>>> &par)
{
    queue<pair<pair<ll, ll>, pair<ll, ll>>> q;
    q.push({{-1, -1}, {a.first, a.second}});
    vis[a.first][a.second] = 1;
    while (!q.empty())
    {
        pair<ll, ll> now = q.front().second;
        pair<ll, ll> parent = q.front().first;
        par[now.first][now.second] = parent;
        q.pop();
        ...
        else
        {
            for (int i = 0; i < 4; i++)
            {
                if (now.first + dx[i] >= 0 && now.first + dx[i] < v.size() && now.second + dy[i] >= 0 && now.second + dy[i] < v[0].size() && v[now.first + dx[i]][now.second + dy[i]] != '#' && vis[now.first + dx[i]][now.second + dy[i]] == 0)
                {
                    q.push({{now.first, now.second}, {now.first + dx[i], now.second + dy[i]}});
...
int main()
{
 ...
 
    {
        ...
        for (int i = 1; i < n; i++)
        {
            ...
            int it = lower_bound(now.begin(), now.end(), v[i]) - now.begin();
