...
int main()
{
    ...
    for (int i = 1; i <= (n - 1); i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (dist[tun[j].first.first] != maxi && dist[tun[j].first.first] + tun[j].second < dist[tun[j].first.second])
            {
                dist[tun[j].first.second] = dist[tun[j].first.first] + tun[j].second;
            }
        }
    }
    ll nowans = -dist[n];
    ...
    for (int j = 0; j < m; j++)
    {
        if (dist[tun[j].first.first] != maxi && dist[tun[j].first.first] + tun[j].second < dist[tun[j].first.second])
        {
            q.push(tun[j].first.second);
            vis[tun[j].first.second] = 1;
        }
    }
    if(q.empty()){
        cout<<nowans<<endl;
