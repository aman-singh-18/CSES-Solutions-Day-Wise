...
bool found = true;
void dfs(vec2 &adj, vec &vis, vec &pvis, vec &par, ll node, ll &classst,ll &cycleen)
{
    for (auto val : adj[node])
    {
        if(found==false) return;
        if (vis[val] == 1 && pvis[val] == 1)
        {
            par[val] = node;
            classst = val;
            cycleen = node;
        ...
        else if (vis[val] == 0)
        {
            ...
            pvis[val] = 1;
            vis[val] = 1;
            dfs(adj, vis, pvis, par, val, classst,cycleen);
        }
    }
     if(found==false) return;
    pvis[node] = 0;
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        if (!vis[i])
        {
            vis[i] = 1;
            pvis[i] = 1;
            dfs(adj, vis, pvis, par, i, cyclest,cycleen);
            if (cyclest != -1)
                break;
        }
    }
    ...
    {
        vec fi;
        fi.push_back(cyclest);
        while (cycleen != cyclest)
        {
            fi.push_back(cycleen);
            cycleen = par[cycleen];
        }
        fi.push_back(cyclest);
