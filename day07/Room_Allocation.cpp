...
void pos(vecp &pa)
{
    ...
    s.insert(pa[0].second);
    for (int i = 1; i < pa.size(); i++)
    {
        if (pa[i].first > *s.begin())
        {
            s.erase(s.begin());
        }
        s.insert(pa[i].second);
    }

    vector<pair<pair<ll, ll>, ll>> now;
    for (int i = 0; i < ori.size(); i++)
    {
        now.push_back({ori[i], i});
    }
    sort(now.begin(), now.end());
    ll size = s.size();
    vec2 ans(size + 1);
    ans[1].push_back(now[0].second);
    multiset<pair<pair<ll, ll>, ll>> ms;
    ms.insert({{now[0].first.second, now[0].second}, 1});
    ll maxi = 1;
    for (int i = 1; i < now.size(); i++)
    {
        pair<pair<ll, ll>, ll> kya = *ms.begin();
        if (now[i].first.first > kya.first.first)
        {
            ms.erase(ms.begin());
            ans[kya.second].push_back(now[i].second);
            ms.insert({{now[i].first.second,now[i].second},kya.second});
        }
        else
        {
            ans[maxi + 1].push_back(now[i].second);
            maxi++;
            ms.insert({{now[i].first.second,now[i].second},maxi});
        }
    }
    vec fi(pa.size());
    for(int i=1;i<ans.size();i++){
        for(auto val:ans[i]) fi[val] = i;
    }
    cout<<size<<endl;
    print(fi);
}
...
int main()
{
...

    {
        ...
        vecp pa(n);
        for (int i = 0; i < n; i++)
            cin >> pa[i].first, cin >> pa[i].second;
        pos(pa);
    }
