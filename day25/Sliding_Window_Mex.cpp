...
ll mexi(pbdb &pb){
    ll st = 0,en=pb.size()-1;
    ll what = 0;
    while(st<=en){
        ll mid = (st+en)/2;
        if(*pb.find_by_order(mid)==mid){
            what = mid+1;
            st = mid + 1;
        }
        else en = mid -1;
...
int main()
{
    ...
    multiset<ll> ms;
    pbdb pb;
    for(int i=0;i<k;i++) pb.insert(v[i]),ms.insert(v[i]);
    ans.push_back(mexi(pb));
    for(int i=k;i<n;i++){
        auto it = ms.find(v[i-k]);
        ms.erase(it);
        auto nowit = ms.find(v[i-k]);
        if(nowit==ms.end()) pb.erase(v[i-k]);
        pb.insert(v[i]);
        ms.insert(v[i]);
        ans.push_back(mexi(pb));
    }
