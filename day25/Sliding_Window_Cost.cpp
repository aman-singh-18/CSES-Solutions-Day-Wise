...
int main()
{
    ...
    for (int i = (k - 1); i < n; i++)
    {
        ...
        pb.insert({v[i], i});
        pll now = *(pb.find_by_order((pb.size() - 1) / 2));
    ...
    {
        if (v[i] <= fm)
            msl.insert(v[i]), suml += v[i];
        else
            msr.insert(v[i]), sumr += v[i];
    }
    vec fi;
    fi.push_back((fm * msl.size() - suml + sumr - (msr.size()) * fm));
    for (int i = k; i < n; i++)
    {
        ll in = v[i];
        ll out = v[i - k];

        if (msl.find(out) != msl.end())
        {
            msl.erase(msl.find(out));
            suml -= out;
        }
        else
        {
            msr.erase(msr.find(out));
            sumr -= out;
        }

        if (!msl.empty() && in <= *msl.rbegin())
        {
            msl.insert(in);
            suml += in;
        }
        ...
        ll med = *msl.rbegin();
        fi.push_back(med * msl.size() - suml + sumr - med * msr.size());
    }
