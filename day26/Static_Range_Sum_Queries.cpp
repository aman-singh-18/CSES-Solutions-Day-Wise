...
void bfs(ll node, ll substr, vec &active, vec2 &adj, map<ll, ll> &used, map<ll, ll> &tow)
{
    queue<pll> pro;
    pro.push({node, substr});
    while (!pro.empty())
    {
        pll now = pro.front();
        pro.pop();
        ...
        curr[now.first] = 1;
        queue<ll> fornow;
        fornow.push(now.first);
        while (!fornow.empty())
        {
            ll node = fornow.front();
            fornow.pop();
        }
    }
}
...
int main()
{
    ...
    while(q--){
        ...
        cin>>l>>r;
        l--,r--;
        ans.push_back(sg.query(0,0,n-1,l,r));
    }
