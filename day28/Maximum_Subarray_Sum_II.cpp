...
int main()
{
    ...
    ll ptr2=n-a;
    ll ptr1 = n-b;
    for(int i=ptr1;i<=ptr2;i++) m.insert(pre[i]);
    
   
    ll idx;
    for(int i=n;i>=1;i--){
        maxi = max(maxi,pre[i]-*m.begin());
        auto it = m.find(pre[ptr2]);
        m.erase(it);
        if(ptr1==0){
            idx = i;
            break;
        }
        else m.insert(pre[ptr1-1]); 
        ptr2--;
        ptr1--;
    }
    idx--;
    ...
    while(ptr2>=ptr1){
       auto it = m.find(pre[ptr2+1]);
       if(it!=m.end()) m.erase(it);
       maxi = max(maxi,pre[idx]-*m.begin());
       idx--;
       ptr2--;
    }
    cout<<maxi<<endl;
