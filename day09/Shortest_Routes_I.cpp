...
void ans(vector<vecp> &adj,vec &dist,ll node){
     ...
     while(!q.empty()){
        ...
        for(auto val:adj[now.second]){
            if(dist[now.second]+val.second<dist[val.first]){
                dist[val.first] = val.second + dist[now.second];
...
int main()
{
    ...
    vec dist(n+1,10000000000000000);
    dist[1] = 0;
    ans(adj,dist,1);
